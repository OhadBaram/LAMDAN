...
export const AlertDialog: React.FC<AlertDialogProps> = ({ children, open, onOpenChange }) => open ? <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100] p-4 backdrop-blur-md"><div onClick={(e) => e.stopPropagation()}>{children}</div></div> : null; // Increased backdrop opacity and blur
export const AlertDialogContent: React.FC<AlertDialogContentProps> = ({ children, className, ...props }) => <div className={`bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md ${className || ''}`} {...props}>{children}</div>; // rounded-xl to rounded-2xl
...
export const AlertDialogFooter: React.FC<AlertDialogFooterProps> = ({ children, ...props }) => <div className="p-4 sm:p-6 bg-slate-50 dark:bg-slate-800/60 flex justify-end gap-3 rounded-b-2xl border-t border-slate-200 dark:border-slate-700" {...props}>{children}</div>; // rounded-b-xl to rounded-b-2xl
