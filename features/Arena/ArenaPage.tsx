...
export function ArenaPage() {
    ...
    const { userProfile, activePersonaId, personas } = useUserSettings();
    ...
    const handleRunArena = async () => {
        if (!prompt.trim() || selectedModelIds.length === 0) {
            openErrorDialog(lang === 'he' ? 'קלט חסר' : 'Missing Input', lang === 'he' ? 'אנא הזן הנחיה ובחר לפחות מודל אחד.' : 'Please enter a prompt and select at least one model.');
            return;
        }
        setIsLoading(true);
        setResults([]); 
    ...
    return (
        <div className="space-y-6">
                                ...
                                {validModels.map(model => (
                                    ...
                                    >
                                        <div className="flex flex-col items-start w-full">
                                           ...
                                           <span className="text-gray-500 dark:text-gray-400 text-[10px]">{model.modelId}</span>
                                        </div>
                                    </Button>
                                ))}
                            </div>
                        ): (
                             <p className="text-sm text-gray-500">{lang === 'he' ? 'אין מודלים תקינים זמינים. אנא בדוק את הגדרות ה-API.' : 'No valid models available. Please check API settings.'}</p>
                        )}
                    </div>
                </CardContent>
                <CardFooter>
                    <Button onClick={handleRunArena} disabled={isLoading || !prompt.trim() || selectedModelIds.length === 0}>
                        {isLoading ? <Loader2 className="w-4 h-4 animate-spin me-2"/> : <Zap className="w-4 h-4 me-2"/>}
            ...
            {results.length > 0 && (
                <div className={`grid grid-cols-1 md:grid-cols-2 ${selectedModelIds.length > 2 ? 'lg:grid-cols-' + Math.min(selectedModelIds.length, 4) : ''} gap-4`}>
                    {results.map(result => (
                        <Card key={result.modelId}>
                            <CardHeader>
                                <CardTitle className="text-md">{result.name} <span className="text-xs text-gray-400">({result.provider})</span></CardTitle>
                            ...
                            <CardContent className="text-sm max-h-96 overflow-y-auto whitespace-pre-wrap">
                                {result.error ? <p className="text-red-500">{result.error}</p> : <p>{result.response}</p>}
                            </CardContent>
                        </Card>
                    ))}
                </div>
