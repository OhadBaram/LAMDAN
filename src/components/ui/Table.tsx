...
export const TableHead: React.FC<TableHeadProps> = ({ children, className, ...props }) => <th className={`text-left px-4 py-3 text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider ${className || ''}`} {...props}>{children}</th>;
...
export const TableCell: React.FC<TableCellProps> = ({ children, className, ...props }) => <td className={`px-4 py-3 text-sm text-[var(--text-primary)] ${className || ''}`} {...props}>{children}</td>;
